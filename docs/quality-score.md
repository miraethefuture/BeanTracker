...
| Persistence and sync | 1 | Runtime storage is still `InMemoryDatabase`; no SwiftData or CloudKit implementation exists. | Ship a live persistence adapter behind `DatabaseClient`. |
...
- persistence and sync are not implemented
