...
BeanTracker는 홈카페 사용자가 원두 소비를 최소 입력으로 기록하고, 각 원두로 몇 잔을 마셨는지와 월별 커피 루틴을 직관적으로 확인할 수 있게 해주는 개인 대시보드 앱입니다.
...
- `Projects/Features/*`: 대시보드, 추출 기록, 원두 창고, 온보딩
- `Projects/Domain/CoffeeDomain`: 순수 모델, 계산 로직, fixtures, domain tests
- `Projects/Core/DatabaseClient`: 의존성 클라이언트, SwiftData live 구현, preview/test용 인메모리 구현

## 현재 상태 메모

- 핵심 지표: 현재 활성 원두의 누적 잔 수
- 핵심 경험: 원두 등록 -> 추출 기록 -> 이 원두로 N잔째 확인 -> 원두 소진 시 총 잔 수 확인
- 기술 스택 목표: SwiftUI, TCA, Tuist, SwiftData, CloudKit
- 현재 `DatabaseClient.liveValue`는 샘플 seed 없는 로컬 SwiftData 저장소를 사용합니다.
- CloudKit 동기화는 아직 후속 작업입니다.
